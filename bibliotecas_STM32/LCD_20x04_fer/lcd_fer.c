...
void LCD_Init(void)
{
	...
	HAL_GPIO_WritePin(LCD_PORT, RS_Pin, 0);//LCD_PORT->CLR = LCD_RS;
	HAL_GPIO_WritePin(LCD_PORT, EN_Pin, 0);//LCD_PORT->CLR = LCD_EN;
...
void LCD_SendNibble(uint8_t theNibble)
{
	...
	if (theNibble & 0x01) HAL_GPIO_WritePin(LCD_PORT, D4_Pin, 1);//(LCD_PORT->SET = LCD_D4);
	else			   	  HAL_GPIO_WritePin(LCD_PORT, D4_Pin, 0);//(LCD_PORT->CLR = LCD_D4);

	if (theNibble & 0x02) HAL_GPIO_WritePin(LCD_PORT, D5_Pin, 1);//(LCD_PORT->SET = LCD_D5);
	else			      HAL_GPIO_WritePin(LCD_PORT, D5_Pin, 0);//(LCD_PORT->CLR = LCD_D5);

	if (theNibble & 0x04) HAL_GPIO_WritePin(LCD_PORT, D6_Pin, 1);//(LCD_PORT->SET = LCD_D6);
	else			   	  HAL_GPIO_WritePin(LCD_PORT, D6_Pin, 0);//(LCD_PORT->CLR = LCD_D6);

	if (theNibble & 0x08) HAL_GPIO_WritePin(LCD_PORT, D7_Pin, 1);//(LCD_PORT->SET = LCD_D7);
	else			      HAL_GPIO_WritePin(LCD_PORT, D7_Pin, 0);//(LCD_PORT->CLR = LCD_D7);

	HAL_GPIO_WritePin(LCD_PORT, EN_Pin, 1);//LCD_PORT->SET = LCD_EN;
...
{
	HAL_GPIO_WritePin(LCD_PORT, RS_Pin, 1);//LCD_PORT->SET = LCD_RS;
