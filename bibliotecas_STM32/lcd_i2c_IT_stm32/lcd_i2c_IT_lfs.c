...
void lcd_send_cmd_buff (char cmd)
{
  char data_u, data_l;
	uint8_t data_t[4];
	data_u = (cmd&0xf0);
	...
	data_t[3] = data_l|0x08;  //en=0, rs=0
//	HAL_I2C_Master_Transmit (i2c_handler, SLAVE_ADDRESS_LCD,(uint8_t *) data_t, 4, 100);
	_add_to_lcdBuff((uint8_t*)data_t,4);
}


void lcd_send_data_buff (char data)
{
	char data_u, data_l;
	uint8_t data_t[4];
	data_u = (data&0xf0);
...
void lcd_cursor_buff(int x, int y)
{
    switch (y){
    	case 0:
    		lcd_send_cmd_buff(LCD_DDRAM_ADDRESS + LCD_START_LINE1 + x);
    	break;
    	case 1:
    		lcd_send_cmd_buff(LCD_DDRAM_ADDRESS + LCD_START_LINE2 + x);
    	break;
    	case 2:
    		lcd_send_cmd_buff(LCD_DDRAM_ADDRESS + LCD_START_LINE3 + x);
    	break;
    	case 3:
    		lcd_send_cmd_buff(LCD_DDRAM_ADDRESS + LCD_START_LINE4 + x);
    	break;
    }
}
...
{
	while (*str) lcd_send_data_buff(*str++);
}


void _add_to_lcdBuff(uint8_t* data, uint8_t sizeData){
	uint8_t i = 0;
	while (i < sizeData){
...
void lcd_upDate_IT(void){
	if (flag_sendLcd) lcd_sendBuff_IT();
}
